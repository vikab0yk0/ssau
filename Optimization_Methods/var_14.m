...
fprintf('Точность вычислений: %e\n\n', tol);

fprintf('Метод Дихотомии\n');
[x_min, f_min, calls, iters] = dichotomy_1d(f, a, b, eps, tol, 1000);
fprintf('Минимум: x = %.5f; f(x) = %.5f\n', x_min, f_min);
fprintf('Количество итераций: %d; вызовов: %d\n', iters, calls);

fprintf('\nМетод золотого сечения\n');
[x_min, f_min, calls, iters] = golden_ratio_1d(f, a, b, tol, 1000);
fprintf('Минимум: x = %.5f; f(x) = %.5f\n', x_min, f_min);
fprintf('Количество итераций: %d; вызовов: %d\n', iters, calls);

fprintf('\nМетод Фибоначчи\n');
[x_min, f_min, calls, iters] = fibonacci_1d(f, a, b, tol, 1000);
fprintf('Минимум: x = %.5f; f(x) = %.5f\n', x_min, f_min);
fprintf('Количество итераций: %d; вызовов: %d\n', iters, calls);
